...
export class ApiService {
  ...
  getUploadById(id: number): Observable<ReportData> {
    return this.http.get<ReportData>(`${this.baseUrl}/history/${id}`);
  }

  getUploadHistory(): Observable<UploadHistory[]> {
    return this.http.get<UploadHistory[]>(`${this.baseUrl}/history`);
  }
  ...
  getLatestUpload(): Observable<UploadHistory & { revenue?: any }> {
    return this.http.get<UploadHistory & { revenue?: any }>(
      `${this.baseUrl}/history/latest`
    );
  }

  getReportByUploadId(id: number): Observable<ReportData> {
    return this.http.get<ReportData>(`${this.baseUrl}/report/${id}`);
  }

  //RPH changes
  getRPH(): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}/rph`);
  }

  saveRPH(data: any[]): Observable<any> {
    return this.http.post(`${this.baseUrl}/rph`, data);
  }

  updateRPH(id: number, data: any): Observable<any> {
    return this.http.put(`${this.baseUrl}/rph/${id}`, data);
  }

  deleteRPH(id: number): Observable<any> {
    return this.http.delete(`${this.baseUrl}/rph/${id}`);
  }

  //REVENUE
  getLatestRevenue(): Observable<any> {
    return this.http.get(`${this.baseUrl}/revenue/latest`);
  }
}
